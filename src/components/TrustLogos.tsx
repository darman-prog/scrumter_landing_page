      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
        ...
        <ul className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
          {copy.logos.map((logo) => (
            <li
              key={logo}
              className="flex min-h-16 items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 text-sm font-black text-slate-400 grayscale transition hover:-translate-y-1 hover:text-slate-700 hover:shadow-lg hover:grayscale-0 dark:border-white/10 dark:bg-slate-950/50 dark:text-slate-500 dark:hover:text-white"
            >
              {logo}
            </li>
          ))}
